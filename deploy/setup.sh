...
sed -e "s/User=pi/User=$(whoami)/" -e "s|%h|$HOME|g" deploy/bot.service | sudo tee /etc/systemd/system/personal-team-bot.service > /dev/null
sed -e "s/User=pi/User=$(whoami)/" -e "s|%h|$HOME|g" deploy/scheduler.service | sudo tee /etc/systemd/system/personal-team-scheduler.service > /dev/null
